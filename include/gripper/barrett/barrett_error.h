...
public:
    ...
    std::string message(int condition) const override {
        ...
        // Decode bitmask
        std::string msg;
        if (condition & static_cast<int>(BarrettHandError::NO_MOTOR_BOARD_FOUND))   msg += "No motor board found; ";
        if (condition & static_cast<int>(BarrettHandError::NO_MOTOR_FOUND))          msg += "No motor found; ";
        if (condition & static_cast<int>(BarrettHandError::MOTOR_NOT_INITIALIZED))    msg += "Motor not initialized; ";
        if (condition & static_cast<int>(BarrettHandError::COULDNT_REACH_POSITION))   msg += "Couldn't reach position; ";
        if (condition & static_cast<int>(BarrettHandError::UNKNOWN_COMMAND))          msg += "Unknown command; ";
        if (condition & static_cast<int>(BarrettHandError::UNKNOWN_PARAMETER_NAME))   msg += "Unknown parameter name; ";
        if (condition & static_cast<int>(BarrettHandError::INVALID_VALUE))           msg += "Invalid value; ";
        if (condition & static_cast<int>(BarrettHandError::WRITE_TO_READ_ONLY))       msg += "Tried to write a read-only property; ";
        if (condition & static_cast<int>(BarrettHandError::TOO_MANY_ARGUMENTS))       msg += "Too many arguments; ";
        if (condition & static_cast<int>(BarrettHandError::INVALID_REALTIME_HEADER))  msg += "Invalid RealTime header; ";
        if (condition & static_cast<int>(BarrettHandError::INVALID_MOTOR_PREFIX))     msg += "Invalid motor prefix; ";
        if (condition & static_cast<int>(BarrettHandError::OVERTEMPERATURE_FAULT))    msg += "Overtemperature fault; ";
        if (condition & static_cast<int>(BarrettHandError::CTRL_C_ABORT))             msg += "Ctrl-C abort received; ";
